...
public class TreasureMapRecursive extends Map {
	 ...
	 * @param map int [][] treasure map
	 * @param x int width to evaluate
	 * @param y int height to evaluate
	 * @param depth how many distance points to evaluate
	 * @return int number of nearby treasures at this point.
	 */
	private int solvePointRecursive(int [][] map, int x, int y, int depth){
		int count = 0;
		
		if(depth == 0){
			if(isTreasurePoint(map, x,y)){
				count=1;
			}
		}else{
			depth--;
			for (int i = x - 1; i < x + 2; i++) {
				...
				for (int j = y - 1; j < y + 2; j++) {
					...
					count += solvePointRecursive(map, i, j, depth);
	...
	protected int solvePoint(int [][] map, int x, int y) {
		return solvePointRecursive(map, x, y, 1);
	}
